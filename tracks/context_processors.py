...
def ui_track_state(request):
    ...
    Globally expose:
      ...
      - my_collection_ids: union of (own tracks) ∪ (in any of my albums) ∪ (explicitly saved)
    """
    ...
    saved_ids = list(
        SavedTrack.objects.filter(owner=user).values_list("original_track_id", flat=True)
    )
